...
public class VerProcesoController {
	...
	public void actualizarProceso() {
		try {
			...
			if(_proceso.getDemandante().getId_persona().equals(_screen.getDemandante().getId_persona()))
				cambio = true;
			if(_proceso.getDemandado().getId_persona().equals(_screen.getDemandado().getId_persona()))
				cambio = true;
			if(_proceso.getJuzgado().getId_juzgado().equals(_screen.getJuzgado().getId_juzgado()))
				cambio = true;
			...
			if (cambio) {
				_proceso = new Proceso(_proceso.getId_proceso(), _screen.getDemandante(), _screen.getDemandado(), _screen.getFecha(), _screen.getJuzgado(), _screen.getRadicado(), _screen.getRadicadoUnico(), _screen.getActuaciones(), _screen.getEstado(), _screen.getCategoria(), _screen.getTipo(), _screen.getNotas(), _screen.getCampos(), _screen.getPrioridad());
