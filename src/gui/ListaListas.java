...
/*
 * public void drawListRow(ListField list, Graphics g, int index, int y, int w)
 * { if (String.class.isInstance(this.get(this, index))) { int posX = getWidth()
 * / 2 - getContentWidth() / 4; int posY = getRowHeight() / 2 -
 * getFont().getHeight() / 2; g.drawText((String) this.get(this, index), posX,
 * posY); } else { Persona objeto = (Persona) this.get(this, index); String
 * nombre = objeto.getNombre(); String id = objeto.getId(); String notas =
 * objeto.getNotas();
 * 
 * g.setFont(getFont().derive(Font.SANS_SERIF_STYLE)); g.drawText(nombre, 0, y);
 * g.setFont(getFont().derive(Font.SANS_SERIF_STYLE, fuente2));
 * g.setColor(colorFuente2); g.drawText(id, 0, y + fuente1); g.drawText(notas,
 * 0, y + fuente1 + fuente2); } g.setColor(colorLinea); g.drawLine(0, y + height
 * - 1, getWidth(), y + height - 1); }
 */