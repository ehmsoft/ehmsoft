...
public class ActuacionesManager extends Manager{
	...
	public ActuacionesManager(int cant){
		...
		_juzgado = new LabelField("", LabelField.RIGHT | LabelField.USE_ALL_WIDTH);
		_fecha = new LabelField("", LabelField.RIGHT | LabelField.USE_ALL_WIDTH);
		_fechaProxima = new LabelField("", LabelField.RIGHT | LabelField.USE_ALL_WIDTH);
		_descripcion = new LabelField("", LabelField.RIGHT | LabelField.USE_ALL_WIDTH);
		...
		Border border = BorderFactory.createRoundedBorder(new XYEdges(r, r, r, r));
	...
	void rightUpdate() {
		...
		if(a == null) {
			...
			String fecha = ListadoActuacionesLista.calendarToString(a.getFecha());
			fecha = fecha.substring(0, 10) + "\n" + fecha.substring(11);
			_fecha.setText(fecha);
			fecha = ListadoActuacionesLista.calendarToString(a.getFechaProxima());
			fecha = fecha.substring(0, 10) + "\n" + fecha.substring(11);
	...
	protected void sublayout(int width, int height) {
		...
		layoutChild(_title, (width / 2) - p, getFont().getHeight() + (p/2));
		layoutChild(_vertical, (width /2) - p, height - (getFont().getHeight() * 2));
		
		//setPositionChild(_lista, 0, 0);
		setPositionChild(_manager, 0, 0);
		setPositionChild(_title, (width / 2) + p, 0);
		setPositionChild(_vertical, (width / 2) + p, getFont().getHeight() * 2 + ((p/2)+p));
		
		setExtent(Display.getWidth() - (w*2), Display.getHeight() - h - (w*2));
	}
}
...
class ListadoManager extends Manager {
	...
	protected void sublayout(int width, int height) {
		...
		setPositionChild(_lista, 0, 0);
		setExtent((Display.getWidth() - (w*2))/2 - p, Display.getHeight() - h - (w*2) - 2*p);
	}
}
